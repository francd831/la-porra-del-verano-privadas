...
-- Recreate with additional group_order_bonus column in return
...
BEGIN
  ...
  FROM predictions p WHERE p.user_id = p_user_id AND p.match_id LIKE 'R32_%' AND p.predicted_winner_team_id IS NOT NULL
  ...
  FROM predictions p WHERE p.user_id = p_user_id AND p.match_id LIKE 'R16_%' AND p.predicted_winner_team_id IS NOT NULL
  ...
  FROM predictions p WHERE p.user_id = p_user_id AND p.match_id LIKE 'QF_%' AND p.predicted_winner_team_id IS NOT NULL
  ...
  FROM predictions p WHERE p.user_id = p_user_id AND p.match_id LIKE 'SF_%' AND p.predicted_winner_team_id IS NOT NULL
...
-- Update the update_all_user_points function to also save points_group_order
...
BEGIN
  ...
  FOR v_user_submission IN SELECT user_id FROM user_submissions WHERE tournament_id = p_tournament_id
  LOOP
    ...
    UPDATE user_submissions SET 
      ...
      updated_at = now()
    WHERE user_id = v_user_submission.user_id AND tournament_id = p_tournament_id;
  END LOOP;
END;
$$;