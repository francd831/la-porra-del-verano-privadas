-- Actualizar la función calculate_user_points para incluir el bonus de +20 por acertar el orden del grupo
...
BEGIN
  ...
  LOOP
    ...
    IF v_all_group_matches_completed THEN
      -- Get admin's official order (from group_standings_override if exists, otherwise calculate)
      ...
      -- If no manual override, calculate order from actual match results
      IF v_admin_order IS NULL THEN
        WITH team_stats AS (
          SELECT 
            COALESCE(m.home_team_id, '') as team_id,
            SUM(CASE 
          ...
          FROM matches m
          WHERE m.tournament_id = p_tournament_id AND m.match_type = 'group' AND m.group_id = v_group_id AND m.status = 'completed'
          GROUP BY m.home_team_id
          UNION ALL
          SELECT 
            COALESCE(m.away_team_id, '') as team_id,
            SUM(CASE 
        ...
        aggregated_stats AS (
          SELECT team_id, SUM(points) as points, SUM(gd) as gd, SUM(gf) as gf
          FROM team_stats
          WHERE team_id != ''
          GROUP BY team_id
      ...
      -- Calculate user's predicted order based on their predictions
      WITH user_team_stats AS (
        SELECT 
          COALESCE(m.home_team_id, '') as team_id,
          SUM(CASE 
            WHEN p.home_goals > p.away_goals THEN 3
            WHEN p.home_goals = p.away_goals THEN 1
            ELSE 0
          END) as points,
        ...
        FROM predictions p
        INNER JOIN matches m ON p.match_id = m.id
        WHERE p.user_id = p_user_id AND m.tournament_id = p_tournament_id AND m.match_type = 'group' AND m.group_id = v_group_id
        ...
        SELECT 
          COALESCE(m.away_team_id, '') as team_id,
          SUM(CASE 
            WHEN p.away_goals > p.home_goals THEN 3
            WHEN p.away_goals = p.home_goals THEN 1
            ELSE 0
          END) as points,
      ...
      -- Compare orders - if they match exactly, add 20 points
  ...
  -- Add group order bonus to groups points
